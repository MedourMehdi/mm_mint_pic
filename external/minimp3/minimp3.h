...
static const L12_subband_alloc_t *L12_subband_alloc_table(const uint8_t *hdr, L12_scale_info *sci)
{
    ...
    int nbands, stereo_bands = (mode == MINIMP3_MODE_MONO) ? 0 : (mode == MODE_JOINT_STEREO) ? (HDR_GET_STEREO_MODE_EXT(hdr) << 2) + 4 : 32;
    ...
    } else
    {
        ...
        unsigned kbps = hdr_bitrate_kbps(hdr) >> (int)(mode != MINIMP3_MODE_MONO);
        if (!kbps) /* free-format */
        {
            kbps = 192;
        }
