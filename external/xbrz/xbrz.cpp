...
#ifdef __MINT__
    return sqrt((2 + r_avg / 255) * square(r_diff) + 4 * square(g_diff) + (2 + (255 - r_avg) / 255) * square(b_diff));
#else
...
#ifdef __MINT__
    return sqrt(square(lumaWeight * y) + square(c_b) +  square(c_r));
#else
